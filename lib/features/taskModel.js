...
MemoryPersistence.prototype.taskById = function(taskId, callback){
	return callback(this[taskId]);
};

MemoryPersistence.prototype.remove = function(taskId, callback){
	return callback(delete this[taskId]);
};

MemoryPersistence.prototype.collect = function(generation, callback){
	...
	return callback(ids.filter(function(id){ return delete this[id]; }.bind(this)));
};

MemoryPersistence.prototype.save = function(taskid, task, callback){
	this[taskid] = task;
	return callback();
};
	...
	this.persistence.taskById(taskId, function(task){
		if(task) switch(task.state){
			...
			case TaskState.Completed:
				this.persistence.remove(taskId, function(success){
					this.count -= 1;
...
ActiveTaskCollection.prototype.collect = function(generation){
	this.persistence.collect(generation, function(ids){
		this.count -= ids.length;
	}.bind(this));
};

ActiveTaskCollection.prototype.enqueue = function(taskId, task, args){
	if(this.count >= this.capacity) return false;
	this.persistence.save(taskId, task, function(){
		this.count += 1;

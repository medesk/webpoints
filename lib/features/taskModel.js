...
HttpAsyncFlow.prototype.beginInvoke = function(remainingTime, callback){
	...
	if(remainingTime > this.maxRemainingTime) return (this.callback(utils.format('The task has too much run-time(%s ms).', remainingTime), 400), callback());
	//pushes a new task into the queue
	if(this.task) return callback();
		...
		if(task) task.setProperties({state: AsyncState.Executed, startedAt: new Date()}, function(success){
			if(success){
				...
				return callback(this.endInvoke.bind(this));
			}
			else return callback();
		}.bind(this));
		else return (this.callback(utils.format('The capacity(%s) of the task pool exceeded.', this.queue.capacity), 400), callback());
	}.bind(this));
};
...
TaskScheduler.prototype.task = function(handler, options){
	...
	function autoAsyncHandler(args, callback){
		...
		computeRemainingTime(this, appraise, args, function(remainingTime){
			...
			flow.beginInvoke(remainingTime, function(callback){

//Module dependencies
var express = require('express'), utils = require('util');
...
Application.prototype.createOperation = function(url, descriptor, server){
	if(typeof descriptor == 'string' || descriptor instanceof String)	//static file hosting
		...
		server[descriptor.method || 'get'](url, execServiceOperation.bind({'environment': this.environment, 'operation': descriptor, 'responseCallback': this.createResponseCallback}));
...
Application.prototype.run = function(port, middleware){
	...
	//Setup middleware
	(middleware || []).forEach(function(f){ app.use(f); });

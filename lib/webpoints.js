...
Application.prototype.createResponseCallback = function(operation, response){
	...
	if(operation.serialize == true) return function(value, statuscode, headers){ 
		...
		if(value instanceof EventEmitter && value.readable) //Readable stream expected
			return value.pipe(response);
		else if(value instanceof Buffer)		//Buffer expected
			return response.end(value);
		else if(statuscode < 300 || statuscode === undefined || statuscode === null)	//JSON serialization
			return response.send(value !== undefined ? JSON.stringify(value) : '', statuscode);
		else
			response.send(value || '', statuscode);	//Raw message
	};
	//custom serializer

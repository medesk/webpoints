...
Application.prototype.createResponseCallback = function(operation, response){
	...
	if(operation.serialize == true) return function(value, statuscode, headers){ 
		...
		//for successfull status codes I should serialize the return value
		return statuscode < 300 || statuscode === undefined || statuscode === null ?
			response.send(value !== undefined ? JSON.stringify(value) : '', statuscode):
			response.send(value || '', statuscode);
	};
	//custom serializer

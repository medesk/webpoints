...
Application.prototype.createResponseCallback = function(operation, response){
	...
	if(operation.serialize == true) return function(value, headers){ 
		if(headers) for(var hdr in headers) response.header(hdr, headers[hdr]);
	...
	//text serializer
		...
		if(headers) for(var hdr in headers) response.header(hdr, headers[hdr]);
		return response.send(value, statuscode);
	};
};
...
/**
 * Prepares service operation handler for execution.
 * 
 * @param request HTTP request.
...
Application.prototype.createOperation = function(url, descriptor, server){
	...
	this.features.forEach(function(feature){ 
		if(feature instanceof Function) feature(descriptor); 
		else feature.apply(descriptor);
	});
...
/**
 * Converts synchronous handler to the asynchronous handler.
 * 
 * @param {Function} f A synchronous handler.
 * @return {Function} An asynchronous handler.
 * @api public
 */
